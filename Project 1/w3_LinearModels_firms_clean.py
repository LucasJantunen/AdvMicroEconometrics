...
def load_firms_data(path: str = 'firms.csv'):
    """Load the balanced firm panel from 'firms.csv' and build (y, X, T, year, labels)."""
    import pandas as pd
    dat = pd.read_csv(path).sort_values(['firmid','year']).reset_index(drop=True)
    required = {'firmid','year','lcap','lemp','ldsa'}
    missing = required - set(dat.columns)
    assert not missing, f"Missing columns in firms.csv: {missing}"
    N = dat['firmid'].nunique()
    T = dat['year'].nunique()
    assert dat.shape[0] == N*T, 'Data is not a balanced panel (N*T != rows).'
    y = dat['ldsa'].to_numpy().reshape(-1,1)
    X = np.column_stack([np.ones((N*T,)), dat['lemp'].to_numpy(), dat['lcap'].to_numpy()])
    year = dat['year'].to_numpy().reshape(-1,1)
    label_y = 'ldsa'; label_x = ['const','lemp','lcap']
    return y, X, T, year, label_y, label_x

if __name__ == "__main__":
    y, X, T, year, label_y, label_x = load_firms_data("firms.csv")
    res = estimate(y, X, transform='', T=T, robust_se=False)
    print_table((label_y, label_x), res, title="Pooled OLS on firms.csv", floatfmt=".4f")
